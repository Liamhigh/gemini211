...
const Message: React.FC<MessageProps> = ({ message, onSealReport, onSealAndEmailReport, isEnterpriseMode }) => {
  ...
  const bubbleClasses = `p-4 rounded-lg break-words ${isUser ? 'bg-blue-600 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-white'}`;
    ...
    <div className={containerClasses}>
      <div>
        ...
        {message.verificationResult && (
             <div className="mt-2 text-xs text-green-600 dark:text-green-400 flex items-start justify-end space-x-2 p-2 bg-green-50 dark:bg-green-900/20 rounded-md">
                <ShieldCheckIcon className="h-5 w-5 flex-shrink-0 mt-0.5" />
                <div className="text-right">
                    <div className="font-semibold">{message.verificationResult.consensusText} ({message.verificationResult.verifiedCount}/{message.verificationResult.totalVoters} Voters)</div>
                    {message.verificationResult.notes && (
                      <div className="text-gray-500 dark:text-gray-400 text-[10px] italic mt-1 whitespace-pre-wrap">
                          Notes: {message.verificationResult.notes}
                      </div>
                    )}
                </div>
            </div>
